...
const checkToken = {
    ...
    isLoggedin: async(req, res, next) => {
        ...
        if (!token) {
            ...
            if (user == -3) {
                //유효기간이 지난 토큰일 때
                return res.json(util.successFalse(statusCode.DONE_TOKEN,resMessage.EXPRIED_TOKEN));
            } else if (user == -2) {
                //잘못 형식의 토큰(키 값이 다르거나 등등)일 때
                return res.json(util.successFalse(statusCode.NOT_TOKEN_FORMAT, resMessage.INVALID_TOKEN));
            } else {
